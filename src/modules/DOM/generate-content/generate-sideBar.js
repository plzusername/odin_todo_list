...
function createSideBarSection(){
    ...
    const project_add_label=createElement({class:'add-project-input', id:'project_input'}, 'input', '', [])
    const project_add_input=createElement({class:'add-project-input', for:'project_input'}, 'label', 'Add Project', [])

    const project_input=createElement({class:'add-project-input'}, 'div', '', [project_add_label,project_add_input])
    const add_icon=createElement({class:'project-add-icon fa-solid fa-plus'}, 'i', '',[])

    const add_project_section=createElement({class:'add-project-label'}, 'div', '', [project_input,add_icon])
