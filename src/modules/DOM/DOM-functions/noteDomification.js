...
function noteDomification(note){
    ...
    const noteTitle=createElement({class:'note-title'}, 'p', '', [titleText])
    ...
    const leftSide=createElement({class:'note-left-side'}, 'div', '', [noteText])
    const rightSide=createElement({class:'note-right-side'}, 'div', '', [noteDelete, noteEdit])

    const noteContainer=createElement({class:'note-container'}, 'div', '', [leftSide,rightSide])
